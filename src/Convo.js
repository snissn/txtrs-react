...
export default class Convo extends React.Component {
  ...
  onSendSecretMessage = async (event) => {
    ...
    var send = await private_message.methods
      .alice_send_encrypted_message(encrypt.toString("hex"))
  ...
  render() {
    ...
    return (
      <Card bsStyle="info" key={message.id} className="centeralign">
        <Card.Header as="h3">
          {(() => {
            switch (message.stage) {
              ...
              case "2":
                return "Recipient has shared one time use encryption keys with you. Time to encrypt and send the message!";
              case "3":
        ...
        >
          <div>
            <p>conversation wtih</p>
            <p>{message.bob}</p>
            <p>{message.id} total messages</p>
          </div>
        </Card.Body>
      </Card>
    );
