...
import { getContract, contract, contractws, w3, users_address, getPrivateMessage, getBlockNumber, private_message_bob_stage_2 } from "./Web3Helper"
...
export default class ReceivedMessages extends React.Component {
  ...
  async getPrivateMessages() {
    var messages_count = await contractws.methods.get_received_messages_total(users_address).call();
    var messages = []
    for (var index = messages_count - 1; index >= 0; index--) {
      var private_message_addr = await contractws.methods.get_received_message(users_address, index).call()
